...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-right-column">
		...
		<div id="image-block">
		{if $have_image}
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large')}" {if $jqZoomEnabled}class="jqzoom" alt="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox')}"{else} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" {/if} id="bigpic" width="300" height="300"/>
		{else}
			<img src="{$img_prod_dir}{$lang_iso}-default-large.jpg" alt="{$product->name|escape:'htmlall':'UTF-8'}" title="{$product->name|escape:'htmlall':'UTF-8'}" />
		{/if}
		</div>
	...
	<div id="pb-left-column">
		...
		<form id="buy_block" action="{$base_dir}cart.php" method="post">
			...
			<p class="price">
				...
				{/if}-->
					<span id="our_price_display">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}&nbsp;{l s='tax excl.'}</span>
...
<div id="more_info_block" class="clear">
	...
	{if $features}
		...
		*}
		...
		<tbody>
		{foreach from=$features item=feature}
			<tr class="{cycle values="even,odd"}"><th>{$feature.name|escape:'htmlall':'UTF-8'}</</th><td>{$feature.value|escape:'htmlall':'UTF-8'}</td></tr>
		{/foreach}
		</tbody>
		</table>
	{/if}
