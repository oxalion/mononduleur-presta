...
{/if}
{if !$priceDisplay || $priceDisplay == 2} {* TTC *}
	{assign var='productPrice' value=$product->getPrice(true, $smarty.const.NULL, $priceDisplayPrecision)}
	{assign var='productPriceHT' value=$product->getPrice(false, $smarty.const.NULL, $priceDisplayPrecision)}
	{assign var='productPriceWithoutReduction' value=$product->getPriceWithoutReduct(false, $smarty.const.NULL)}
{elseif $priceDisplay == 1} {* HT *}
...
<div id="primary_block" class="clearfix" itemscope itemtype="http://data-vocabulary.org/Product">
	...
	<div id="pb-right-column">
		...
		{if $have_image}
			<span id="view_full_size">
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')}"{if $jqZoomEnabled} class="jqzoom"{/if} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" id="bigpic" width="{$largeSize.width}" height="{$largeSize.height}" itemprop="image" />
	...
	<div id="pb-left-column">
		<h1>
			<span itemprop="name">{$product->name|escape:'htmlall':'UTF-8'}</span>
			 {if $product->manufacturer_name}
			 	<span class="pdt-by-brand"> {l s='By'} </span>
					<a href="{$link->getmanufacturerLink($product->id_manufacturer, $manufacturer.link_rewrite)|escape:'htmlall':'UTF-8'}" title="{$product->manufacturer_name|escape:'htmlall':'UTF-8'}">
						<span class="pdt-brand" itemprop="brand">{$product->manufacturer_name|escape:'htmlall':'UTF-8'}</span>
					</a>
			 {/if}
		</h1>
		...
		<!-- add to cart form-->
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post" itemprop="offerDetails" itemscope itemtype="http://data-vocabulary.org/Offer">
			...
			{else}
				<p id="availability_statut">
					...
					<span id="availability_value"{if $product->quantity <= 0} class="warning_inline"{/if}>
						...
						{else}
							<span itemprop="availability" content="in_stock">{$product->available_now}</span>
						{/if}
					</span>
				</p>
			{/if}
		...
		<div class="content_prices clearfix">
			...
			<div class="price">
				...
				{if $priceDisplay >= 0 && $priceDisplay <= 2}
					...
					<!-- <span class="hors-tax"><span itemprop="price">{convertPrice price=$productPriceHT}</span> {l s='tax excl.'}</span>
					<br /><span class="ttc" style="display: inline;">{convertPrice price=$productPrice} {l s='tax incl.'}</span> -->

					{if $priceDisplay == 1}
						<span class="hors-tax">
							<span itemprop="price" id="pretaxe_price_display">{convertPrice price=$productPrice}</span>
					...
					{else}
						<span class="hors-tax">
						<span id="pretaxe_price_display">{convertPrice price=$productPriceHT}</span>
							{if $tax_enabled  && ((isset($display_tax_label) && $display_tax_label == 1) OR !isset($display_tax_label))}
								{l s='tax excl.'}
							{/if}
						</span><br />
...
<div id="more_info_block" class="clear">
	...
	{if isset($product) && $product->description}
		...
		<div id="idTab1" class="rte">
			{$product->description}
			{hook h='productMoreInfoTab'}
		</div>
	{/if}
	{if isset($features) && $features}
		<!-- product's features -->
		<table id="idTab2" class="fiche-tech">
			<tbody>
			{foreach from=$features item=feature name=listFeature}
	            {if isset($feature.value)}
				    <tr{if $smarty.foreach.listFeature.index % 2 == 0} class="alt"{/if}><th>{$feature.name|escape:'htmlall':'UTF-8'}</th><td>{$feature.value|escape:'htmlall':'UTF-8'}</td></tr>
	            {/if}
			{/foreach}
			</tbody>
		</table>
	...
	{if isset($attachments) && $attachments}
		...
		{foreach from=$attachments item=attachment}
			<li><a href="{$link->getPageLink('attachment', true, NULL, "id_attachment={$attachment.id_attachment}")}">{$attachment.name|escape:'htmlall':'UTF-8'}</a></li>
		{/foreach}
		</ul>
	{/if}
