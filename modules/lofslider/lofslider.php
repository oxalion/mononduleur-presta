...
class lofslider extends Module
{
    ...
    */
	...
	{ 
		...
		$this->context->controller->addJS(__PS_BASE_URI__.'modules/lofslider/assets/script.js');        		        
	...
	function processHook($pparams, $pos="home"){
		...
		if( $homeSorce == "selectcat"){				
			...
			if(_PS_VERSION_ <="1.4"){				
				...
				$products = self::getProductsV15( $where,0,$params->get("limit_items",5),"p.id_product" );
			}			
	    ...
	    return $this->display(__FILE__, 'tmpl/'.$this->getParamValue('module_theme','default').'/default.tpl').$content;					
	}
